...
function RegisterContent() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (!targetEmail) {
      const inputEmail = window.prompt("Enter your Google Account email address (e.g. user@gmail.com):", "user@gmail.com");
      if (!inputEmail) return;
      targetEmail = inputEmail.trim();
    }
    if (!targetName) {
      targetName = targetEmail.split('@')[0].replace('.', ' ').toUpperCase();
    }
    setLoading(true);
    setError('');
    setInfoMessage(`🌐 Creating account for ${targetEmail}...`);

    try {
      await registerUser(targetName, targetEmail, "google_sso_pass", role);
      setLoading(false);
      window.location.href = `/login?registered=true&email=${encodeURIComponent(targetEmail)}`;
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4 sm:p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-slate-950/90 backdrop-blur-xl rounded-3xl p-8 border border-purple-900/60 shadow-2xl space-y-6 relative z-10">
        ...
        <button
          ...
          className="w-full py-3 px-4 rounded-xl bg-slate-900 hover:bg-slate-850 text-slate-200 border border-slate-700 font-bold text-xs transition-all shadow-sm flex items-center justify-center gap-3 cursor-pointer group"
        >
        ...
        <div className="relative flex items-center justify-center my-4">
          ...
          <span className="bg-slate-950 px-3 text-[10px] uppercase font-bold text-slate-500 tracking-wider">or register with details</span>
        </div>
